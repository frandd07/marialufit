...
export async function SAVE_MEASURE(user_id, peso, fecha) {
    const { data, error } = await supabase
        .from('medida') // Suponiendo que tienes una tabla "medida"
        .insert([{ user_id: user_id, peso: peso, fecha: fecha }]); // Usa 'fecha' aquí
...
export async function GET_MEASURES(user_id) {
    const { data, error } = await supabase
        .from('medida')
        .select('peso, fecha') // Asegúrate de que 'peso' y 'fecha' son los nombres correctos
        .eq('user_id', user_id) // Filtra por el ID del usuario
